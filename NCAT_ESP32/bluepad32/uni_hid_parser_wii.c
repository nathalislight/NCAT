...
static void process_drm_k_horizontal(uni_gamepad_t* gp, const uint8_t* data) {
    ...
    gp->buttons |= (data[1] & 0x02) ? BUTTON_A_ : 0;  // Button "1"
    gp->buttons |= (data[1] & 0x01) ? BUTTON_B_ : 0;  // Button "2"
    gp->updated_states |=
...
static void process_drm_k_vertical(uni_gamepad_t* gp, const uint8_t* data) {
    ...
    // buttons
    gp->buttons |= (data[1] & 0x04) ? BUTTON_A_ : 0;  // Shoulder button
    gp->buttons |= (data[1] & 0x08) ? BUTTON_B_ : 0;  // Big button "A"
...
static void process_drm_ka(uni_hid_device_t* d, const uint8_t* report, uint16_t len) {
    ...
    gp->buttons |= (report[2] & 0x08) ? BUTTON_A_ : 0;  // Big button "A"
    gp->buttons |= (report[2] & 0x04) ? BUTTON_B_ : 0;  // Button Shoulder
...
static void process_drm_ke(uni_hid_device_t* d, const uint8_t* report, uint16_t len) {
    ...
    if (ins->flags == WII_FLAGS_VERTICAL) {
        ...
        gp->buttons |= n.bc ? BUTTON_A_ : 0;
        gp->buttons |= n.bz ? BUTTON_B_ : 0;
    }
...
static void process_drm_kee(uni_hid_device_t* d, const uint8_t* report, uint16_t len) {
    ...
    gp->buttons |= !(data[9] & 0x10) ? BUTTON_B_ : 0;           // BA
    gp->buttons |= !(data[9] & 0x40) ? BUTTON_A_ : 0;           // BB
...
static void process_drm_e(uni_hid_device_t* d, const uint8_t* report, uint16_t len) {
    ...
    // Buttons A,B,X,Y
    gp->buttons |= (data[5] & 0b01000000) ? 0 : BUTTON_A_;
    gp->buttons |= (data[5] & 0b00010000) ? 0 : BUTTON_B_;
...
void uni_hid_parser_wii_set_player_leds(uni_hid_device_t* d, uint8_t leds) {
    ...
    ins->gamepad_seat = (uni_gamepad_seat_t)leds;
