...
static uni_hid_device_t* g_devices[UNI_HID_DEVICE_MAX_DEVICES]; /// needs malloc/ps_malloc
...
void uni_hid_device_init(void) {
    ...
    for (uint8_t tmp=0;tmp<UNI_HID_DEVICE_MAX_DEVICES;tmp++) memset(g_devices[tmp], 0, sizeof(uni_hid_device_s)); ///
}
...
//--------------------------------------------------------------------------------
#ifdef MYUNIHID
      d->report_parser.parse_raw = MY_uni_hid_parser_generic_parse_raw;
      logi("Device set as custom generic driver.\n", type);
#else 
//--------------------------------------------------------------------------------
...
void uni_hid_device_send_intr_report(uni_hid_device_t* d, const uint8_t* report, uint16_t len) {
    ...
    uni_hid_device_send_report(d, d->conn.interrupt_cid, (const uint8_t*)report, len); ///
}
...
void uni_hid_device_send_ctrl_report(uni_hid_device_t* d, const uint8_t* report, uint16_t len) {
    ...
    uni_hid_device_send_report(d, d->conn.control_cid, (const uint8_t*)report, len); ///
}
...
void uni_hid_device_send_queued_reports(uni_hid_device_t* d) {
    ...
    uni_hid_device_send_report(d, cid, (const uint8_t*)data, data_len); ///
}
