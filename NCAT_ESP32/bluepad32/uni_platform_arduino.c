...
static void arduino_on_device_disconnected(uni_hid_device_t* d) {
    ...
    if (ins->gamepad_idx != UNI_ARDUINO_GAMEPAD_INVALID) {
        ...
        _gamepad_seats = (uni_gamepad_seat_t)((int)_gamepad_seats & ~(1 << ins->gamepad_idx) );
...
static int arduino_on_device_ready(uni_hid_device_t* d) {
    if (_gamepad_seats == (uni_gamepad_seat_t)(GAMEPAD_SEAT_A | GAMEPAD_SEAT_B | GAMEPAD_SEAT_C | GAMEPAD_SEAT_D)) {
    ...
    for (int i = 0; i < ARDUINO_MAX_GAMEPADS; i++) {
        if ((_gamepad_seats & (1 << i)) == 0) {
            ins->gamepad_idx = i;
            _gamepad_seats = (uni_gamepad_seat_t)((int)_gamepad_seats | (1 << i));
            break;
        }
    }
...
/*    pending_request_t request = (pending_request_t){
    ...
    pending_request_t request; ///
        request.gamepad_idx = idx; ///
        request.cmd = PENDING_REQUEST_CMD_PLAYER_LEDS; ///
        request.args[0] = leds; ///
...
/*    pending_request_t request = (pending_request_t){
    ...
    pending_request_t request; ///
        request.gamepad_idx = idx; ///
        request.cmd = PENDING_REQUEST_CMD_LIGHTBAR_COLOR; ///
        request.args[0] = r; ///
        request.args[1] = g; ///
        request.args[2] = b; ///
...
/*    pending_request_t request = (pending_request_t){
    ...
    pending_request_t request; ///
        request.gamepad_idx = idx; ///
        request.cmd = PENDING_REQUEST_CMD_RUMBLE; ///
        request.args[0] = force; ///
        request.args[1] = duration; ///
