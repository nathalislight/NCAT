...
static uint8_t* alloc_pool ( /* Pointer to allocated memory block (NULL:no memory available) */
  JDEC* jd_,       /* Pointer to the decompressor object */
...
)
{
  ...
  if (jd_->sz_pool >= nd) {
    jd_->sz_pool -= nd;
    rp = jd_->pool;     /* Get start of available memory pool */
    jd_->pool =  jd_->pool + nd;  /* Allocate requierd bytes */
  }
...
static JRESULT mcu_output (
  JDEC* jd,     /* Pointer to the decompressor object */
  int (*outfunc)(JDEC*, uint16_t*, JRECT*), /* RGB output function */
...
)
{
  ...
  return outfunc(jd, (uint16_t*)jd->workbuf, &rect) ? JDR_OK : JDR_INTR;
}
...
JRESULT jd_prepare (
  ...
  uint8_t* pool,       /* Working buffer for the decompression session */
...
)
{
  ...
  seg = alloc_pool(jd, JD_SZBUF);   /* Allocate stream input buffer */
  jd->inbuf = seg;
