...
/* derived from Konstantin Chizhov's AVR port templates */
  ...
  extern uint8_t USB_HOST_CS;
  extern uint8_t USB_HOST_INT;
  extern uint8_t USB_HOST_MISO;
  extern uint8_t USB_HOST_MOSI;
  extern uint8_t USB_HOST_SCK;
...
//slower but better compatibility in shared devices?
...
public: \
  ...
  static uint8_t IsSet() { \
    if (pin!=-1) return digitalRead(pin); else return 1;  \
  } \
};
...
public: \
  ...
  static void SetDirRead() { \
    PIN_INPUT_ENABLE(GPIO_PIN_MUX_REG[pin]); PIN_INPUT_ENABLE(GPIO_PIN_MUX_REG[pin]); GPIO.enable_w1tc = (1 << pin); \
  } \
  static void SetDirWrite() { \
    GPIO.enable_w1ts = (1 << pin); PIN_INPUT_DISABLE(GPIO_PIN_MUX_REG[pin]); PIN_INPUT_DISABLE(GPIO_PIN_MUX_REG[pin]); \
  } \
  static uint8_t IsSet() { \
...
MAKE_PIN(P0, 1);          ///CAN NOT BE ZERO = already used...
MAKE_PIN(P1, 1); // TX0
MAKE_PIN(P10, 10); // TX1
MAKE_PIN(P3, 3); // RX0
...
MAKE_PIN(P19, USB_HOST_MISO); // MISO
MAKE_PIN(P23, USB_HOST_MOSI); // MOSI
MAKE_PIN(P18, USB_HOST_SCK); // SCK
MAKE_PIN(P5, USB_HOST_CS); // SS 
MAKE_PIN(P17, USB_HOST_INT); // INT
