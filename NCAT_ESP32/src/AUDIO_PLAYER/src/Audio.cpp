...
///#define I2S_BCLK (GPIO_NUM_12) //BCK
///#define I2S_LRC  (GPIO_NUM_22) //LCK 
///#define I2S_DI_IO -1           //Not Used

extern bool player_I2S_enabled;

//------------------------------------------------------------------------------
 
extern uint16_t AUDIO_WRITE(uint8_t* buffer_audio,uint16_t bytes_to_write,uint32_t AUDIO_delay);
extern uint8_t AUDIO_SET_PARAMS(uint16_t sampRate, uint8_t bits, uint8_t channels,uint8_t AUDIOCORE=0);
...
uint32_t incomingByte = 65; // for incoming serial data
...
//*******************************************************************************
                if (player_I2S_enabled) {
                    ...
                    int16_t n_Sample[256];
                    uint16_t configtmp=256;
                    for (uint32_t tmp=0;tmp<configtmp;tmp++) {
                        n_Sample[tmp]=Gain(m_outBuff[m_curSample * 2 + tmp]);
                    }
 
                    AUDIO_WRITE((uint8_t *)n_Sample, configtmp*2,portMAX_DELAY);

                    m_validSamples-=configtmp/2;
                    m_curSample+=configtmp/2;	
//*******************************************************************************
                } else {				   

                    ///m_Sample[LEFTCHANNEL]  = Gain(m_outBuff[m_curSample * 2]);
                    ///m_Sample[RIGHTCHANNEL] = Gain(m_outBuff[m_curSample * 2 + 1]);

                    uint16_t n_Sample[64];
                    uint16_t configtmp=64;
                    for (uint32_t tmp=0;tmp<configtmp;tmp++) {
                    ...
                    AUDIO_WRITE((uint8_t *)n_Sample, configtmp*2,portMAX_DELAY);
       			    ...
       			    m_i2s_bytesWritten=2*configtmp;
...
bool Audio::setSampleRate(uint32_t sampRate) {
    ...
    AUDIO_SET_PARAMS(sampRate, 16, 2); ///
...
if (player_I2S_enabled) {
    ...
    err=i2s_write((i2s_port_t)m_i2s_num, (const char*)&s32, sizeof(uint32_t), &m_i2s_bytesWritten, 1000);
...
//---------------------------------------------------------------------------------------------------------------------
uint16_t Audio::Gain_alt(uint16_t s) {
    int32_t v;
    v= (s * m_vol)>>6;
