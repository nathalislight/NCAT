...
#define CUSTOM_CACHE_SEGMENT SOC_EXTRAM_DATA_LOW ///is default: 0x3f800000 
...
#define VIRT_HIMEM_RANGE_START (CUSTOM_CACHE_SEGMENT+(128-SPIRAM_BANKSWITCH_RESERVE)*CACHE_BLOCKSIZE)
...
static void set_bank(int virt_bank, int phys_bank, int ct)
{
    ...
    int r=1;
    r = cache_sram_mmu_set_2( 1, 0, CUSTOM_CACHE_SEGMENT + CACHE_BLOCKSIZE * virt_bank, phys_bank * CACHE_BLOCKSIZE, 32, ct );
    ///assert(r == 0);
    while(r !=0) {}; ///halt
    r = cache_sram_mmu_set_2( 0, 0, CUSTOM_CACHE_SEGMENT + CACHE_BLOCKSIZE * virt_bank, phys_bank * CACHE_BLOCKSIZE, 32, ct );
    ///assert(r == 0);
    while(r !=0) {}; ///halt


/*  int r=1;
    r = cache_sram_mmu_set( 0, 1000, CUSTOM_CACHE_SEGMENT + CACHE_BLOCKSIZE * virt_bank, phys_bank * CACHE_BLOCKSIZE, 32, ct );
    ///assert(r == 0);
    while(r !=0) {};
    r = cache_sram_mmu_set( 1, 1000, CUSTOM_CACHE_SEGMENT + CACHE_BLOCKSIZE * virt_bank, phys_bank * CACHE_BLOCKSIZE, 32, ct );
    ///assert(r == 0);
    while(r !=0) {};
*/
}
...
esp_err_t esp_himem_map(esp_himem_handle_t handle, esp_himem_rangehandle_t range, size_t ram_offset, size_t range_offset, size_t len, int flags, uint8_t **out_ptr)
{
    ...
    //Set out pointer
    *out_ptr = (uint8_t **)(VIRT_HIMEM_RANGE_START + (range->block_start + range_offset) * CACHE_BLOCKSIZE);
    return ESP_OK;
}

esp_err_t esp_himem_unmap(esp_himem_rangehandle_t range, uint8_t *ptr, size_t len)
{
