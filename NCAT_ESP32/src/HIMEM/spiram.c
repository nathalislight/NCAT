...
static int spiram_size_usable_for_malloc2()
{
    int s=esp_spiram_get_size2();
...
bool esp_spiram_test2()
{
    volatile int *spiram=(volatile int*)SOC_EXTRAM_DATA_LOW;
    size_t p;
    size_t s=spiram_size_usable_for_malloc2();
    int errct=0;
    int initial_err=-1;
    ...
    if (errct) {
        ESP_EARLY_LOGE(TAG2, "SPI SRAM memory test fail. %d/%d writes failed, first @ %X\n", errct, s/32, initial_err+SOC_EXTRAM_DATA_LOW);
        return false;
    } else {
        ESP_EARLY_LOGI(TAG2, "SPI SRAM memory test OK");
        return true;
    }
}

void ///IRAM_ATTR 
esp_spiram_init_cache2()
{
...
esp_spiram_size_t esp_spiram_get_chip_size2()
{
    if (!spiram_inited) {
        ESP_EARLY_LOGE(TAG2, "SPI RAM not initialized");
...
esp_err_t esp_spiram_init2()
{
    esp_err_t r;
    r = psram_enable(PSRAM_SPEED, PSRAM_MODE);
    if (r != ESP_OK) {
#if CONFIG_SPIRAM_IGNORE_NOTFOUND
        ESP_EARLY_LOGE(TAG2, "SPI RAM enabled but initialization failed. Bailing out.");
#endif
        return r;
    }

    spiram_inited=true; //note: this needs to be set before esp_spiram_get_chip_esp_spiram_get_size calls
#if (CONFIG_SPIRAM_SIZE != -1)
    if (esp_spiram_get_size2()!=CONFIG_SPIRAM_SIZE) {
        ESP_EARLY_LOGE(TAG2, "Expected %dKiB chip but found %dKiB chip. Bailing out..", CONFIG_SPIRAM_SIZE/1024, esp_spiram_get_size2()/1024);
        return ESP_ERR_INVALID_SIZE;
    }
#endif

    ESP_EARLY_LOGI(TAG2, "Found %dMBit SPI RAM device",
                                          (esp_spiram_get_size2()*8)/(1024*1024));
    ESP_EARLY_LOGI(TAG2, "SPI RAM mode: %s", PSRAM_SPEED == PSRAM_CACHE_F40M_S40M ? "flash 40m sram 40m" : \
                                          PSRAM_SPEED == PSRAM_CACHE_F80M_S40M ? "flash 80m sram 40m" : \
                                          PSRAM_SPEED == PSRAM_CACHE_F80M_S80M ? "flash 80m sram 80m" : "ERROR");
    ESP_EARLY_LOGI(TAG2, "PSRAM initialized, cache is in %s mode.", \
...
esp_err_t esp_spiram_add_to_heapalloc2()
{
    //Add entire external RAM region to heap allocator. Heap allocator knows the capabilities of this type of memory, so there's
    //no need to explicitly specify them.
#if CONFIG_SPIRAM_ALLOW_BSS_SEG_EXTERNAL_MEMORY
    ESP_EARLY_LOGI(TAG2, "Adding pool of %dK of external SPI memory to heap allocator", (spiram_size_usable_for_malloc2() - (&_ext_ram_bss_end - &_ext_ram_bss_start))/1024);
    return heap_caps_add_region((intptr_t)&_ext_ram_bss_end, (intptr_t)SOC_EXTRAM_DATA_LOW + spiram_size_usable_for_malloc2()-1);
#else
    ESP_EARLY_LOGI(TAG2, "Adding pool of %dK of external SPI memory to heap allocator", spiram_size_usable_for_malloc2()/1024);
    return heap_caps_add_region((intptr_t)SOC_EXTRAM_DATA_LOW, (intptr_t)SOC_EXTRAM_DATA_LOW + spiram_size_usable_for_malloc2()-1);
#endif
}
...
esp_err_t esp_spiram_reserve_dma_pool2(size_t size) {
    ESP_EARLY_LOGI(TAG2, "Reserving pool of %dK of internal memory for DMA/internal allocations", size/1024);
    ...
    while (size > 0) {
        ...
        ESP_EARLY_LOGD(TAG2, "Allocating block of size %d bytes", next_size);
        dma_heap = (uint8_t*)heap_caps_malloc(next_size, MALLOC_CAP_DMA|MALLOC_CAP_INTERNAL);
...
size_t esp_spiram_get_size2()
{
    esp_spiram_size_t size=esp_spiram_get_chip_size2();
    if (size==(esp_spiram_size_t)PSRAM_SIZE_16MBITS) return 2*1024*1024;
    if (size==(esp_spiram_size_t)PSRAM_SIZE_32MBITS) return 4*1024*1024;
    if (size==(esp_spiram_size_t)PSRAM_SIZE_64MBITS) return 8*1024*1024;
...
void esp_spiram_writeback_cache2()
{
    int x;
    volatile int i=0;
 ...
 */
bool esp_spiram_is_initialized2()
{
