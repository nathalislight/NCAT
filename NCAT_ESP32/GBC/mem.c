...
#if 1
	map[0xC] = ram_ibank[0] - 0xC000;
	n = R_SVBK & 0x07;
	map[0xD] = ram_ibank[n?n:1] - 0xD000;
	map[0xE] = ram_ibank[0] - 0xE000;
	map[0xF] = NULL;
#else
	map[0xC] = NULL;
	map[0xD] = NULL;
...
if (GBC_IN_HIMEM) {
   ...
   if (bootromLoaded==1)  ///boot rom loaded
{
  map[0x0] = bootrom;
  map[0x1] = bootrom;
  map[0x2] = bootrom;
...
/*	map[0x0] = GBC_HIMEM_BANK0_BUFFER; //getRomBank(0);
	map[0x1] = GBC_HIMEM_BANK0_BUFFER; //getRomBank(0);
	map[0x2] = GBC_HIMEM_BANK0_BUFFER; //getRomBank(0);
	map[0x3] = GBC_HIMEM_BANK0_BUFFER; //getRomBank(0);
	*/
}	
	if (mbc.rombank < mbc.romsize)
	{
...
//--------------------------------------------------------------------------------
		...
		case RI_BCPS:
		R_BCPS = b & 0xBF;
		R_BCPD = GBC_lcd->pal[b & 0x3F];
		break;
		case RI_OCPS:
		R_OCPS = b & 0xBF;
		R_OCPD = GBC_lcd->pal[64 + (b & 0x3F)];
		break;
		case RI_BCPD:
		R_BCPD = b;
...
/*		__asm__("nop");
		...
		if ((a & 0xF000) == 0xC000)
		{
			ram_ibank[0][a & 0x0FFF] = b;
			break;
		}
		n = R_SVBK & 0x07;
		ram_ibank[n?n:1][a & 0x0FFF] = b;
		break;
		case 0xE:
		if (a < 0xFE00)
		{
		...
		if ((a & 0xFF00) == 0xFE00)
		{
			...
			if (a < 0xFEA0) GBC_lcd->oam.mem[a & 0xFF] = b;
			break;
		}
...
//--------------------------------------------------------------------------------
if (GBC_IN_HIMEM) {
	    if (bootromLoaded==1) return bootrom[a];
        else 
...
/*		__asm__("nop");
		...
		case 0xC:
		if ((a & 0xF000) == 0xC000)
			return ram_ibank[0][a & 0x0FFF];
		n = R_SVBK & 0x07;
		return ram_ibank[n?n:1][a & 0x0FFF];
		case 0xE:
		if (a < 0xFE00) return mem_read(a & 0xDFFF);
		if ((a & 0xFF00) == 0xFE00)
		{
			/* if (R_STAT & 0x02) return 0xFF; */
			if (a < 0xFEA0) return GBC_lcd->oam.mem[a & 0xFF];
			return 0xFF;
		}
...
return 0; ///NCX: some sound_read problem... disabled for now.
