...
{
	{  0, 0,(byte)-1, 0, 0, 0, 0, 0 },
	{  0,(byte)-1,(byte)-1, 0, 0, 0, 0, 0 },
	{ (byte)-1,(byte)-1,(byte)-1,(byte)-1, 0, 0, 0, 0 },
	{ (byte)-1, 0, 0,(byte)-1,(byte)-1,(byte)-1,(byte)-1,(byte)-1 }
};
...
void sound_reset()
{
    pcm.hz=GBC_SAMPLERATE; /// *= slowing, /=speeding
    pcm.pos=0;
    pcm.len=GBC_FRAGSIZE; ///
...
int16_t* GBC_SAMPLE=&audio_frame[BUFIDX1%8][(pcm.pos%GBC_FRAGSIZE)*2+0];


///int16_t* GBC_RIGHT =&audio_frame[BUFIDX1%8][(pcm.pos%GBC_FRAGSIZE)*2+0]; ///
///int16_t* GBC_LEFT =&audio_frame[BUFIDX1%8][(pcm.pos%GBC_FRAGSIZE)*2+1]; ///

	int s, l, r, f, n;

  if (!RATE || cpu__GBC.snd < RATE) return;
  
  if (gbcaudio_busy==1) return;

gbcaudio_busy=1;

	for (; cpu__GBC.snd >= RATE; cpu__GBC.snd -= RATE) ///
	{
		l = r = 0;
...
///    audio_frame[BUFIDX1%8][(pcm.pos%GBC_FRAGSIZE)*2]=r<<4; ///
///    audio_frame[BUFIDX1%8][(pcm.pos%GBC_FRAGSIZE)*2+1]=l<<4; ///



    pcm.pos+=1;

    if (pcm.pos>=GBC_FRAGSIZE) {

while (pcm_busy) delayMicroseconds(1); // need wait to pcm_submit() complete...

///        gbcaudio_busy=1;
        BUFSIZE[BUFIDX1%8]=GBC_FRAGSIZE;
        BUFIDX1++;
            while (!pcm_busy && BUFIDX2+7<BUFIDX1) {
                printf(">>> BUFFER OVERLOAD TUNING SOURCE ___ BUFIDX1:%d BUFIDX2: %d\n",BUFIDX1,BUFIDX2);
                ///vTaskDelay(1); ///
	            vPortYield();  ///THIS IS REALLY REQUIRED HERE FOR MULTITASKING (ESPECIALY BLUETOOTH)
	            BUFIDX2++;
            }
		    ///pcm.pos=pcm.pos%GBC_FRAGSIZE;
            pcm.pos=0; ///

			///GBC_SAMPLE=&audio_frame[BUFIDX1%8][(pcm.pos%GBC_FRAGSIZE)*2+0]; ///
