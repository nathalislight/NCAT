...
void hw_interrupt(byte i, byte mask)
{
	...
	if ((R_IF & (R_IF ^ oldif) & R_IE) && cpu__GBC.ime) cpu__GBC.halt = 0;
	// if ((i & (hw.ilines ^ i) & R_IE) && cpu__GBC.ime) cpu__GBC.halt = 0; 
	// if ((i & R_IE) && cpu__GBC.ime) cpu__GBC.halt = 0; 
...
///	cpu_timers((460>>cpu__GBC.speed)+cnt*16); dalias
